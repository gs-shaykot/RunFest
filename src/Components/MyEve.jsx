...
const MyEve = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:5111/marathons/email/?contactEmail=${user.email}`, { withCredentials: true })
            .then(response => {
    ...
    const handleEveDelete = id => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:5111/marathons/all/${id}`)
                    .then(response => {
    ...
    const handleUpdate = (e) => {
        ...
        axios.put(`http://localhost:5111/marathons/all/${selectedID}`, formVals)
            .then(response => {
                Swal.fire({
                    title: "Succeess",
