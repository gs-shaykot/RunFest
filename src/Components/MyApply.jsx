...
const MyApply = () => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`http://localhost:5111/applied/candidate?email=${user.email}&search=${search}`, { withCredentials: true })
            .then(response => {
    ...
    const handleAppliedDelete = id => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:5111/applied/all/${id}`,)
                    .then(response => {
    ...
    const handleUpdate = (e) => {
        ...
        axios.put(`http://localhost:5111/applied/all/${selectedID}`, formVals)
            .then(response => {
                Swal.fire({
                    title: "Succeess",
